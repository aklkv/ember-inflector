import Inflector from '#src/index.ts';
import type { InflectionRule } from '#src/index.ts';
import { module, test } from 'qunit';

module('Unit | Utility | inflector', function (hooks) {
  ...
  test('inflect.advancedRules', function (assert) {
    ...
    const inflector = new Inflector();
    const rules: InflectionRule[] = [[/^(ox)$/i, '$1en']];
  ...
  test('Inflector.defaultRules', function (assert) {
    ...
    const rules = Inflector.defaultRules;
